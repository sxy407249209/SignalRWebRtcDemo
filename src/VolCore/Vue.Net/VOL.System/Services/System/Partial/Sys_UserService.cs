...
using VOL.Entity.DomainModels.System.Dto;
...
namespace VOL.System.Services
{
    public partial class Sys_UserService
    {
        ...
        public override PageGridData<Sys_User> GetPageData(PageDataOptions pageData)
        {
            ...
            List<Sys_UserDto> Sys_UserDtos = new List<Sys_UserDto>();
            foreach (var userinfo in gridData.rows)
            {
                string isonline = "离线";
                foreach (var conn in SignalRConnManagement.Connection)
                {                 
                    if (userinfo.User_Id== conn.UserId)
                    {
                        isonline = "在线";
                    }                   
                }
                Sys_UserDto sys_UserDto = new Sys_UserDto
                {
                ...
                Sys_UserDtos.Add(sys_UserDto);
            }

            //2将Sys_UserDto的数据赋值给gridData.extra
            gridData.extra = Sys_UserDtos;
